-- ============================================================================
-- Retail HRMS — Night Duty two-tier decision RPCs: Operations Manager, then
-- Super Manager
-- Migration 0049
--
-- attendance_night_duty_om_decide(p_approval_id, p_decision, p_manager_
-- payable_out_time, p_remark) — p_decision in ('approved','disallowed',
-- 'carry_forward'). Caller must be an active Operations Manager assigned to
-- the request's store (attendance_operations_manager_assignments) and must
-- not be the employee the request is about. Only acts while
-- approval_status = 'pending_om' — enforced via an atomic
-- `UPDATE ... WHERE approval_status = 'pending_om'` so two OMs assigned to
-- the same store racing on the same request can never both succeed (the
-- second gets 0 rows affected and a clear "already decided" error).
--
-- attendance_night_duty_super_manager_decide(...) — mirrors the same shape:
-- caller must be an active Super Manager (attendance_super_managers, company-
-- wide, not store-scoped), only acts while approval_status =
-- 'pending_super_manager', same atomic-update race protection so multiple
-- Super Managers can never produce two conflicting final decisions on one
-- request.
--
-- Neither RPC ever writes attendance_records.punch_out_at — only
-- payable_working_minutes / payable_overtime_minutes / payable_extra_duty_
-- value / night_duty_approval_id change, exactly as in migration 0045's
-- attendance_night_duty_decide() (which remains in place, untouched, as the
-- Super-Admin emergency-access fallback described in migration 0047).
-- ============================================================================
