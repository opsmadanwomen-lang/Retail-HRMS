-- ============================================================================
-- Retail HRMS — fix attendance_explain_rules() double Information-consumption
-- hazard (migration 0069)
--
-- BUG (found during mandatory testing): attendance_explain_rules() called
-- compute_late_and_penalty_facts() a SECOND time (after compute_extended_
-- attendance_facts() already called it once internally) purely to read the
-- o_is_weekly_off flag, which compute_extended_attendance_facts() does not
-- bubble up. When p_use_information=true and the employee's monthly
-- Information quota had exactly one use remaining, the FIRST call (inside
-- compute_extended_attendance_facts) consumed it and inserted the usage row;
-- the SECOND, redundant call then saw the quota exhausted and RAISED "No
-- Information/Intimation remaining for this month" -- turning a read-only
-- explain request into a hard error, exactly the side-effect hazard the
-- read-only requirement was meant to avoid.
--
-- FIX: stop calling compute_late_and_penalty_facts() a second time entirely.
-- is_weekly_off_on_date(employee_id, date) is the exact same pure, STABLE,
-- side-effect-free helper compute_late_and_penalty_facts() itself calls to
-- derive o_is_weekly_off -- calling it directly gives the identical boolean
-- with zero risk of double-consuming Information usage.
--
-- DEFENSE IN DEPTH: this function must never be able to CREATE a new
-- Information usage row, no matter what a caller passes for p_use_information
-- -- it is a read-only "explain" endpoint, not a punch/save path. p_use_
-- information is now honoured only when a usage row for this employee+date
-- ALREADY exists (in which case compute_late_and_penalty_facts()'s own
-- ON CONFLICT DO NOTHING makes the call a true no-op, needed only to
-- correctly re-derive a historically-genuine Information/noon-cutoff day's
-- Late figure) -- otherwise it is silently downgraded to false. Every other
-- line is unchanged from migration 0068.
-- ============================================================================
...
declare
  ...
  v_safe_use_information boolean;
  ...
  v_used_count int;
begin
  -- Never let this read-only function create a NEW Information usage row -- only honour
  -- p_use_information when a usage row for this exact employee+date already exists (making the
  -- downstream ON CONFLICT DO NOTHING insert a guaranteed no-op).
  select exists(
  ...
  v_safe_use_information := coalesce(p_use_information, false) and v_information_already_used;
  ...
  select * into v_facts from public.compute_extended_attendance_facts(
    ...
    p_punch_in_at, p_punch_out_at, v_safe_use_information, null
  );
  -- Pure, side-effect-free -- the exact same helper compute_late_and_penalty_facts() itself calls
  -- to derive o_is_weekly_off. Never call compute_late_and_penalty_facts() a second time here: it
  -- can insert an Information usage row, and calling it twice risks the second call seeing the
  -- quota the first call just consumed and raising an exception.
