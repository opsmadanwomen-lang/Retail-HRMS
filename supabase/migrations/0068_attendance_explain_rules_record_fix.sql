-- ============================================================================
-- Retail HRMS — fix attendance_explain_rules() "record not assigned" error
-- (migration 0068)
--
-- BUG (found during mandatory testing, never reached production): when a rule
-- kind had no resolved rule at all (v_rule_id is null), the scope-lookup
-- block was skipped entirely, leaving the `record`-typed v_row variable in
-- Postgres's "not yet assigned" state (assigning NULL to a bare `record`
-- variable does not give it a tuple structure). Reading v_row.scope_source
-- afterward then raised "record v_row is not assigned yet" for every
-- Not-Assigned kind — i.e. this broke on the single most common case
-- (Overtime/Half Day/Extended Duty simply not configured yet).
--
-- FIX: replace the single `record` variable with five separately-typed
-- scalar variables (text/uuid/uuid/date/date), reset to NULL at the top of
-- every loop iteration. Scalars default to NULL cleanly with no "not
-- assigned" state, so reading them when no scope row was found is always
-- safe. No other logic changes -- the four-tier lookup queries themselves
-- and every fact/config/result computation are byte-for-byte identical to
-- migration 0067.
-- ============================================================================
