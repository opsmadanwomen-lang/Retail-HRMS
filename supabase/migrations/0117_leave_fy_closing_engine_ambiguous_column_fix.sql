-- ============================================================================
-- Retail HRMS — Leave Management, Phase 4: fix ambiguous column references
-- in leave_compute_fy_closing()
-- Migration 0117
--
-- BUG FOUND DURING LIVE TESTING: RETURNS TABLE(...) declares output
-- parameters named employee_id/leave_type_id/policy_id — identical to real
-- column names on leave_policy_type_configs/leave_encashment_rules/
-- leave_ledger queried inside the loop. Postgres raised
-- "42702: column reference is ambiguous" the moment this actually ran,
-- since a bare `where policy_id = v_policy_id` could mean either the output
-- parameter or the table column. Fixed by qualifying every such reference
-- with its table name. Purely a qualifier fix — no calculation logic
-- changed.
-- ============================================================================
        ...
        if v_remaining > 0 then
          ...
          if v_cfg.encashment_allowed then
            ...
            if v_encash.id is not null and v_encash.enabled then
              select * into v_salary from public.leave_resolve_salary_components(v_emp.id, v_fy.end_date);
