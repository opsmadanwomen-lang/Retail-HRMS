-- ============================================================================
-- Retail HRMS — Fix: payroll_policy_finalize with ZERO deduction lines produced
--   a JSON `null` for "ordered", and the callers (payroll_apply_policy,
--   payroll_policy_preview) then did jsonb_array_elements(v_fin -> 'ordered')
--   -> ERROR "cannot extract elements from a scalar".
--
-- Root cause: when p_deductions is [] the per-row loop never runs, v_rows stays
--   '[]', so `array_agg(...) into arr` returns SQL NULL. to_jsonb(NULL::jsonb[])
--   is SQL NULL, and jsonb_build_object('ordered', NULL) stores JSON null.
--
-- Fix: coalesce the aggregate to an empty jsonb[] so "ordered" is always a JSON
--   array (possibly empty). No behavioural change for any employee that has at
--   least one deduction line. This is what a real ~500-employee payroll run hits
--   for employees with no PF/ESI/PT/LWP/advance and no structure deductions.
-- ============================================================================
