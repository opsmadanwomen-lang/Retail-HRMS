-- ============================================================================
-- Retail HRMS — Leave Management, Phase 4: honest "unresolved" reporting
-- Migration 0119
--
-- BUG FOUND DURING LIVE TESTING (Lapse-disabled scenario): when Carry
-- Forward covers only PART of the available balance and neither Encashment
-- nor Lapse can absorb the rest (e.g. Lapse explicitly disabled for that
-- Leave Type), the engine correctly computed lapse_days=0/encashment_days=0
-- but still labeled the row 'carry_forward_only' — which reads as "fully
-- resolved" when days were actually left unaccounted for. Adds a genuine
-- 'partial_unresolved' status so the Admin UI can flag it honestly instead
-- of implying everything was handled.
-- ============================================================================
alter table public.leave_fy_closing_lines drop constraint if exists leave_fy_closing_lines_final_status_check;
alter table public.leave_fy_closing_lines add constraint leave_fy_closing_lines_final_status_check
  check (final_status in ('carry_forward_only', 'encashed', 'lapsed', 'mixed', 'no_action', 'partial_unresolved'));
...
begin
  ...
  for v_emp in select e.id from public.employees e where e.company_id = v_fy.company_id and e.status = 'active' loop
    ...
    for v_cfg in select c.* from public.leave_policy_type_configs c where c.policy_id = v_policy_id loop
      ...
      v_final_status := case
        when (v_carry_forward + v_encash_days + v_lapse_days) < v_bal.available then 'partial_unresolved'
