-- ============================================================================
-- Retail HRMS — Leave Management, Phase 4: split the gated Prior Notice
-- Exception request into its own RPC
-- Migration 0120
--
-- BUG FOUND DURING LIVE TESTING: for 'hr_approval_required' /
-- 'special_approval_required' / 'emergency_exception', leave_apply() both
-- INSERTed a 'pending' exception row AND RAISEd an exception in the same
-- call to stop the application from being created. In Postgres, a RAISE
-- inside a PL/pgSQL function rolls back every effect that SAME function
-- call made — including its own earlier INSERT — so the exception request
-- was silently discarded every time, never actually reaching HR. Fixed by
-- moving that INSERT into its own standalone RPC
-- (leave_request_prior_notice_exception) — a separate client call commits
-- independently, so the request now genuinely persists. leave_apply() no
-- longer inserts anything for this path; it only raises, pointing the
-- employee at the new RPC. The 'allow_with_reason'/'reject' branches are
-- untouched (they never had this bug — 'allow_with_reason' inserts and then
-- continues to a successful commit; 'reject' never inserts at all).
-- ============================================================================
create or replace function public.leave_request_prior_notice_exception(
...
begin
  ...
  if not coalesce(v_prior_notice.required, false) or v_prior_notice.exception_behavior not in ('hr_approval_required', 'special_approval_required', 'emergency_exception') then
    raise exception 'A Prior Notice exception request is not applicable for this Leave Policy''s current configuration.';
  end if;

  v_notice_days_actual := p_from_date - current_date;
  if v_notice_days_actual >= v_prior_notice.notice_days then
    raise exception 'This application already satisfies the required prior notice — no exception is needed.';
  end if;

  if exists (
    select 1 from public.leave_prior_notice_exceptions
    where employee_id = v_employee_id and leave_type_id = p_leave_type_id and from_date = p_from_date and to_date = p_to_date and status = 'pending'
  ) then
    raise exception 'A prior notice exception request for these dates is already pending.';
  end if;
  ...
  values (v_employee.company_id, v_employee_id, v_prior_notice.exception_behavior, p_reason, 'pending', auth.uid(), p_from_date, p_to_date, p_leave_type_id)
  returning * into v_exception;

  perform public.leave_notify(v_employee.company_id, v_direct_manager_id, 'prior_notice_exception', 'Prior notice exception requested', format('An employee requested a prior-notice exception (Exception ID: %s).', v_exception.id), 'leave_prior_notice_exceptions', v_exception.id);

  return v_exception;
end;
$$;

grant execute on function public.leave_request_prior_notice_exception(uuid, date, date, text) to authenticated;

-- ----------------------------------------------------------------------------
-- leave_apply(): gated behaviors now ONLY raise, pointing at the RPC above —
-- never insert-then-raise in the same call. Every other line byte-identical
-- to 0114.
-- ----------------------------------------------------------------------------
    ...
    if coalesce(v_prior_notice.required, false) then
      ...
      if v_notice_days_actual < v_prior_notice.notice_days then
        ...
        if v_existing_exception.id is not null then
          ...
          -- hr_approval_required / special_approval_required / emergency_exception: the request
          -- must already have been submitted via leave_request_prior_notice_exception() and approved
          -- — this function never inserts one itself (see migration 0120 header note).
