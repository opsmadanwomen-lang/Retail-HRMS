-- ============================================================================
-- Retail HRMS — one shared eligibility rule + a "Test" that covers the WHOLE component (amount AND eligibility)
-- Migration 0182  (additive, idempotent; requires 0178-0181)
--
-- WHY A DATABASE CHANGE IS NEEDED: the Configure screen's Test button could only evaluate the amount formula. Testing the
-- 12-month service condition in the browser would mean re-implementing the eligibility rule in TypeScript (a second engine,
-- and the rule must be decided on the server). So:
--   1. payroll_rule_eligibility(type, months, joining_date, period_end) holds the eligibility rule ONCE
--        eligible  <=>  joining_date + N calendar months <= payroll period end date
--        joining date missing  =>  not eligible + needs_review (never a guess)
--      payroll_policy_compute_lines (payroll + preview + F&F) now CALLS it instead of carrying its own copy. Behaviour is
--      unchanged; only the wording of the Not-applicable note is clearer: "Not applicable — 12 months of service not completed
--      (completes on 01 Oct 2026)."
--   2. payroll_component_test(...) = the amount formula (existing evaluator) + that same eligibility function, for an UNSAVED
--      configuration, with a test-only joining date and payroll month. It reads and writes no employee or payroll data.
-- The 0181 columns (eligibility_type, eligibility_months) are used as they are — no new column. NOT changed: salary slab resolver,
-- structure routing, salary_bifurcate, payroll_calculate_run, OT / LWP / Advance / Attendance. No data is written.
-- ============================================================================

create or replace function public.payroll_rule_eligibility(p_type text, p_months integer, p_joining date, p_period_end date)
returns table (eligible boolean, needs_review boolean, completion_date date, note text)
language plpgsql
immutable
as $fn$
declare v_d date;
begin
  if coalesce(p_type, 'always') <> 'after_months' then
    eligible := true; needs_review := false; completion_date := null; note := null; return next; return;
  end if;
  if p_joining is null then
    eligible := false; needs_review := true; completion_date := null;
    note := format('joining date is missing — the %s-month service condition cannot be checked, so no amount is applied.', p_months);
    return next; return;
  end if;
  v_d := (p_joining + make_interval(months => coalesce(p_months, 0)))::date;   -- calendar months; month-end clamps (29 Feb + 12 months = 28 Feb)
  completion_date := v_d;
  if v_d <= p_period_end then
    eligible := true; needs_review := false; note := null;
  else
    eligible := false; needs_review := false;
    note := format('Not applicable — %s months of service not completed (completes on %s).', p_months, to_char(v_d, 'DD Mon YYYY'));
  end if;
  return next;
end;
$fn$;
revoke execute on function public.payroll_rule_eligibility(text, integer, date, date) from public, anon, authenticated;

-- compute_lines (live body; the inline eligibility block now calls the shared function)
...
create or replace function public.payroll_component_test(
  p_expr text, p_gross numeric, p_basic numeric default null, p_da numeric default null,
  p_eligibility_type text default 'always', p_eligibility_months integer default null,
  p_joining_date date default null, p_payroll_month date default null
)
returns jsonb
language plpgsql
stable
security definer
as $fn$
declare v_err text; v_val numeric; v_el record; v_end date;
begin
  ...
  if coalesce(p_eligibility_type, 'always') not in ('always', 'after_months') then
    return jsonb_build_object('ok', false, 'error', 'Unknown eligibility type.');
  end if;
  ...
  if coalesce(p_eligibility_type, 'always') = 'after_months' then
    if p_eligibility_months is null or p_eligibility_months < 0 or p_eligibility_months > 600 then
      return jsonb_build_object('ok', false, 'error', 'Months of service must be a whole number from 0 to 600.');
    end if;
    if p_payroll_month is null then
      return jsonb_build_object('ok', false, 'error', 'Choose the payroll month to test the service condition.');
    end if;
    v_end := (date_trunc('month', p_payroll_month) + interval '1 month - 1 day')::date;
    select * into v_el from public.payroll_rule_eligibility('after_months', p_eligibility_months, p_joining_date, v_end);
    if not v_el.eligible then
      return jsonb_build_object('ok', true, 'applicable', false, 'value', 0, 'amount_if_eligible', v_val,
        'needs_review', v_el.needs_review, 'note', v_el.note, 'period_end', v_end);
    end if;
    return jsonb_build_object('ok', true, 'applicable', true, 'value', v_val, 'amount_if_eligible', v_val, 'needs_review', false,
      'note', format('Eligible — %s months of service completed on %s (on or before the payroll month end %s).', p_eligibility_months,
                     to_char(v_el.completion_date, 'DD Mon YYYY'), to_char(v_end, 'DD Mon YYYY')), 'period_end', v_end);
  end if;
  return jsonb_build_object('ok', true, 'applicable', true, 'value', v_val, 'amount_if_eligible', v_val, 'needs_review', false, 'note', null);
end;
$fn$;
revoke execute on function public.payroll_component_test(text, numeric, numeric, numeric, text, integer, date, date) from public, anon;
grant execute on function public.payroll_component_test(text, numeric, numeric, numeric, text, integer, date, date) to authenticated;
