-- ============================================================================
-- Retail HRMS — Leave Management, Phase 4: FY Closing calculation engine +
-- Preview + Confirm
-- Migration 0112
--
-- leave_compute_fy_closing(): the ONE authoritative calculation function.
-- Reuses leave_get_balance() (never a second balance formula) and
-- leave_resolve_policy_assignment() (never a second policy resolver) for
-- every employee/leave-type combination. Both leave_preview_fy_closing()
-- (read-only) and leave_confirm_fy_closing() (persists) call this SAME
-- function — there is no separate "closing formula".
--
-- Unresolved balances: if neither Carry Forward, Encashment, nor Lapse ends
-- up applying to a remaining balance (a genuine policy misconfiguration —
-- e.g. encashment disabled AND lapse disabled for a leave type with a
-- balance left over), this engine does NOT invent a default disposition. It
-- reports carry_forward_days/encashment_days/lapse_days honestly (possibly
-- summing to less than `available`) with final_status = 'no_action' — the
-- Admin UI surfaces this as "N days unresolved — check policy
-- configuration" rather than silently lapsing or silently carrying forward
-- something the policy never authorized.
-- ============================================================================
...
begin
  ...
  for v_emp in select e.id from public.employees e where e.company_id = v_fy.company_id and e.status = 'active' loop
    ...
    for v_cfg in select * from public.leave_policy_type_configs where policy_id = v_policy_id loop
      select * into v_bal from public.leave_get_balance(v_emp.id, v_cfg.leave_type_id, p_financial_year_id);

      -- "Opening" is a breakout slice of the SAME 'earned' total leave_get_balance already computes
      -- (which bundles opening + accrual + carry_forward by Phase 2 design) — read directly from the
      -- ledger's own 'opening' rows, never a second balance calculation.
      select coalesce(sum(days), 0) into v_opening
      from public.leave_ledger
      where employee_id = v_emp.id and leave_type_id = v_cfg.leave_type_id and financial_year_id = p_financial_year_id and transaction_type = 'opening';
      ...
      if v_bal.available > 0 then
        ...
        if v_remaining > 0 then
          ...
          if v_cfg.encashment_allowed then
            select * into v_encash from public.leave_encashment_rules where policy_id = v_policy_id;

            if v_encash.id is not null and v_encash.enabled then
              select basic_salary, da into v_salary from public.leave_resolve_salary_components(v_emp.id, v_fy.end_date);

              if v_salary.basic_salary is not null then
                -- Threshold value: only 'basic'/'basic_da' are computable today (no gross/custom
                -- salary data exists anywhere in this codebase — never fabricated).
                if v_encash.threshold_base_type = 'basic' then
                  v_threshold_value := v_salary.basic_salary;
                elsif v_encash.threshold_base_type = 'basic_da' then
                  v_threshold_value := v_salary.basic_salary + coalesce(v_salary.da, 0);
                else
                ...
                if v_encashment_eligible then
                  if v_encash.salary_base_type = 'basic' then
                    v_salary_base := v_salary.basic_salary;
                  elsif v_encash.salary_base_type = 'basic_da' then
                    v_salary_base := v_salary.basic_salary + coalesce(v_salary.da, 0);
                  else
                    v_salary_base := null; -- gross/custom: not computable, falls through below
                  end if;
                  ...
                  if v_salary_base is not null then
                    ...
                    if v_divisor is not null and v_divisor > 0 then
                      ...
                      v_encashment_eligible := false; -- no usable divisor: fall through to lapse
                    end if;
                  else
                    v_encashment_eligible := false;
                  end if;
      ...
      basic_salary_snapshot := v_salary.basic_salary;
      da_snapshot := v_salary.da;
...
grant execute on function public.leave_compute_fy_closing(uuid) to authenticated;

-- ----------------------------------------------------------------------------
-- Preview — read-only, Admin/HR only, NO mutation whatsoever.
-- ----------------------------------------------------------------------------
create or replace function public.leave_preview_fy_closing(p_financial_year_id uuid)
returns table (
  employee_id uuid,
  leave_type_id uuid,
  policy_id uuid,
...
begin
  if not (public.is_super_admin() or public.current_user_role() <> 'staff') then
    raise exception 'Only Admin/HR may preview a Financial Year closing.' using errcode = '42501';
  end if;
  return query select * from public.leave_compute_fy_closing(p_financial_year_id);
end;
$$;
